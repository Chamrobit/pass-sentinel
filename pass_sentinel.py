...
class PasswordGeneratorApp:
    ...
    def setup_ui(self):
        ...
        self.check_frame.place(relx=0.05, rely=0.3, relheight=0.6, relwidth=0.3, anchor='nw')
        ...
        self.specials_box = self.create_checkbox(self.check_frame, self.specials_check_var, 'Specials #-$')
        self.specials_box.place(relx=0.1, rely=0.12, anchor='nw')

        self.digits_box = self.create_checkbox(self.check_frame, self.digits_check_var, 'Numbers 0-9')
        self.digits_box.configure(command=self.enable_digits)
        self.digits_box.place(relx=0.1, rely=0.32, anchor='nw')

        self.lowercase_box = self.create_checkbox(self.check_frame, self.lowercase_check_var, 'Lowercase a-z')
        self.specials_box.configure(command=self.enable_specials)
        self.lowercase_box.place(relx=0.1, rely=0.54, anchor='nw')

        self.uppercase_box = self.create_checkbox(self.check_frame, self.uppercase_check_var, 'Uppercase A-Z')
        self.uppercase_box.place(relx=0.1, rely=0.76, anchor='nw')

        # Sliders for lengths
        self.slider_frame = CTk.CTkFrame(self.root)
        self.slider_frame.place(relx=0.4, rely=0.3, relheight=0.4, relwidth=0.57, anchor='nw')

        self.total_length = CTk.CTkSlider(self.slider_frame, from_=4, to=52, command=lambda e: self.update_label(self.total_length, self.total_length_label, 'Total Length'))
        self.total_length.set(12)
        self.total_length.place(relx=0.99, rely=0.12, relwidth=0.6, anchor='ne')

        self.total_length_label = CTk.CTkLabel(self.slider_frame, text='Total Length: 12')
        self.total_length_label.place(relx=0.05, rely=0.1, anchor='nw')

        self.min_numbers = CTk.CTkSlider(self.slider_frame, from_=0, to=self.total_length.get(), command=lambda e: self.update_label(self.min_numbers, self.min_numbers_label, 'Min Numbers'))
        ...
        self.min_specials = CTk.CTkSlider(self.slider_frame, from_=0, to=self.total_length.get(), command=lambda e: self.update_label(self.min_specials, self.min_specials_label, 'Min Specials'))
        self.min_specials.set(3)
        self.min_specials.place(relx=0.99, rely=0.72, relwidth=0.6, anchor='ne')

        self.min_specials_label = CTk.CTkLabel(self.slider_frame, text='Min Specials: 3')
        self.min_specials_label.place(relx=0.05, rely=0.7, anchor='nw')

        # Password function and button
        self.generate_button = CTk.CTkButton(self.root, text='Generate Password', text_color='black', command=self.generate_password, font=('Calibri', 18))
    ...
    def generate_password(self):
        ...
        lowercase_chars = string.ascii_lowercase if self.lowercase_check_var.get() else ''
        uppercase_chars = string.ascii_uppercase if self.uppercase_check_var.get() else ''
        ...
        if length < min_digits + min_puncts:
            self.password_var.set('Sum of min numbers and min specials\ncan not be larger than length!')
            return
